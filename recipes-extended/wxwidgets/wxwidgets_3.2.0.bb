...
EXTRA_OECMAKE_append_libc-musl = " \
    -DHAVE_LOCALE_T=OFF \
"
EXTRA_OECMAKE_append_class-target = ' -DEGREP="/bin/grep -E"'
...
PACKAGECONFIG_remove_class-native = "opengl"
...
do_install_append() {
    # make the wx-config consistent with the target file-system
    sed -i \
        -e 's:${STAGING_BINDIR_TOOLCHAIN}/::g' \
        -e 's:${RECIPE_SYSROOT}${libdir}/::g' \
        -e 's:${HOSTTOOLS_DIR}/::g' \
        ${D}${libdir}/wx/config/*

    # link the wx-config
    ln -sf ${libdir}/wx/config/gtk3-unicode-3.2 ${D}${bindir}/wx-config

    # install samples
    mkdir -p ${D}${datadir}/${PN}
    cp -r ${S}/samples ${D}${datadir}/${PN}
}
...
FILES_${PN} += "${bindir}/wx-config"
FILES_${PN}-samples = "${datadir}/${PN}/demos ${datadir}/${PN}/samples"
